...
- [x] Phase 6 — GitHub Actions CI fully green for all three apps (Windows .exes + Android .apk)
...
## Downloading the built apps

After a successful Actions run:

1. Open the repo → **Actions** tab
2. Click the latest successful run for the app you want
3. Scroll to **Artifacts** and download:
   - `XamaMaster-windows` → contains `XamaMaster.exe` + `shamm_crypto.dll`
   - `XSeller-windows` → contains `XSeller.exe` + `shamm_crypto.dll` + platform-tools (adb)
   - `Xama-android-apk` → contains the debug-signed `.apk`
